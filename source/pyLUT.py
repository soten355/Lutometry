...
Updated and modified by AJ Young
...
class LUT:
    ...
    def FromCubeFile(cubeFilePath):
        ...
        return LUT(lattice, name = os.path.splitext(os.path.basename(cubeFilePath))[0], resolution = cubeSize)
    ...
    def __add__(self, other):
        if self.resolution is not other.resolution:
            raise NameError("Lattice Sizes not equivalent")
    ...
    def oldPlot(self, type, resolution):
        """
        ...
        lut = None
        if self.cubeSize > resolution:
            lut = self.Resize(resolution)
        else:
            lut = self
        ...
        if (type == "cube"):
            ...
            for code_value in input_range:
                # get a value between [0..1]
                norm_r = code_value/max_value
                norm_g = code_value/max_value
                norm_b = code_value/max_value
                # apply correction
                res = lut.ColorFromColor(Color(norm_r, norm_g, norm_b))
                # append values
                red_values.append(res.r)
        ...
        if (type == "cube"):
            ...
            ax.plot(input_range, red_values, 'o-r',)
            ax.plot(input_range, green_values, 'o-g')
            ax.plot(input_range, blue_values, 'o-b', markevery=0.1)
            grid()
        title(self.name)
        # plot 3D values
        matplotlib.pyplot.show()
    
    def Plot(self, type, resolution):
        """
        Plot a LUT as a 3D RGB cube or 2D graph per line of color using matplotlib.
        Stolen from https://github.com/mikrosimage/ColorPipe-tools/tree/master/plotThatLut.
        """

        # Initialize necessary variables for storing LUT data
        self.cubeSize = range(0, self.resolution)
        max_value = self.resolution - 1.0
        # Process color values
        if (type == "cube"): #If user wants to plot a cube
            for r in self.cubeSize:
                for g in self.cubeSize:
                    for b in self.cubeSize:
                        # get a value between [0..1]
                        norm_r = r/max_value
                        norm_g = g/max_value
                        norm_b = b/max_value
                        # apply correction
                        res = self.ColorFromColor(Color(norm_r, norm_g, norm_b))
                        # append values
                        self.red_values.append(res.r)
                        self.green_values.append(res.g)
                        self.blue_values.append(res.b)
                        # append corresponding color
                        self.colors.append(rgb2hex([norm_r, norm_g, norm_b]))
        elif (type == "graph"): #If user wants to plot a graph
            for code_value in self.cubeSize:
                # get a value between [0..1]
                norm_r = code_value/max_value
                norm_g = code_value/max_value
                norm_b = code_value/max_value
                ...
                res = self.ColorFromColor(Color(norm_r, norm_g, norm_b))
                # append values
                self.red_values.append(res.r*100)
                self.green_values.append(res.g*100)
                self.blue_values.append(res.b*100)

print("pyLUT has loaded!")